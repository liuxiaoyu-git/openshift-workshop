...
Additionally, while still connected to Dev cluster, extract Kafka endpoints:
...
export OCP_DEV_KAFKA_ROUTE=$(oc get route -n $OCP_DEV_KAFKA_NAMESPACE|awk '/bootstrap/ {print $2}'|head -1)
----
